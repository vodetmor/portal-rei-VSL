...
const extendMaterial = (
  ...
  let vert = `${cfg.header ?? ''} ${cfg.vertexHeader ?? ''} ${baseVert}`;
  let frag = `${cfg.header ?? ''} ${cfg.fragmentHeader ?? ''} ${baseFrag}`;

  for (const [inc, code] of Object.entries(cfg.vertex ?? {})) {
    vert = vert.replace(inc, `${inc} ${code}`);
  }
  for (const [inc, code] of Object.entries(cfg.fragment ?? {})) {
    frag = frag.replace(inc, `${inc} ${code}`);
  }
